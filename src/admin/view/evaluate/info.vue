<template>
    <div>
        ...
        <Card class="md-card">
            ...
            <a href="#" slot="extra" class="md-card-btn-success" @click.prevent="setNumberAction">
            ...
            <Card class="md-card">
                <div class="btn-broup">
                    <Button class="btn" @click="closePage" style="margin-right: 80px">返回</Button>
        ...
        <Modal
                :closable="false"
                :mask-closable="false"
                v-model="setNumberModal"
                title="评分配置"
        >
            <Card>
                <Form :model="numberData" ref="numberData" :label-width="120">
                    <FormItem label="满分值 ：" prop="maxNum">
                        <InputNumber
                                placeholder="请填写"
                                :max="10"
                                :min="1"
                                :step="1"
                                :precision="0"
                                v-model="numberData.maxNum"
                        >
                            <span></span>
                        </InputNumber>
                    </FormItem>
                    <FormItem label="差评预警值 ：" prop="maxNum">
                        <InputNumber
                                placeholder="请填写"
                                :max="10"
                                :min="0"
                    ...
                    <FormItem label="是否允许0.5分 ：" prop="isHalf">
                        <RadioGroup v-model="numberData.isHalf">
                            <Radio label="1">是</Radio>
                            <Radio label="0">否</Radio>
                        </RadioGroup>
                    </FormItem>
            ...
            <div slot="footer">
                <Button @click="numberDataOk" type="primary">保存</Button>
            </div>
        </Modal>
    </div>
</template>
    ...
    export default {
        ...
        methods: {
            ...
            numberDataOk() {
                ...
                window.localStorage.numberData = JSON.stringify(this.numberData);
                ...
                if (dimensionContent.length > 0) {
                    ...
                    console.log("dimensionContent", dimensionContent)
                }

                this.setNumberModal = false;
                this.$Modal.success({
                    title: "保存评分配置",
                    content: "设置成功"
                });
        ...
        created() {
            if (window.localStorage.numberData) {
                this.numberData = JSON.parse(window.localStorage.numberData)
            }
            this.getTemplateInfo();
