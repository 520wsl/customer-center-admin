...
const {title, storeageUserInfoKey} = config;
import {forEach, hasOneOf, objEqual} from "@/libs/tools";

export const TOKEN_KEY = "token";

export const getArrMinNumBer = (arr) => {
    let arrMin = arr
    if (arr && arr.length > 0) {
        if (typeof arr === 'string') {
            let newArr = arr.split(",")
            if (newArr.length > 0) {
                arrMin = Math.min.apply(null, newArr);
            }
        }
 ...
 */
export const getValueOfObj = ({arr, content, key, value}) => {
    // 所有参数必填
